...
<!--for employees-->
   ...
   if($res->num_rows>0){
     while($row=$res->fetch_assoc()){
       ...
       echo "<h2 style='text-align:center;color: #666;'>No employee.</h2>";
     }
     echo "</table></div>";
     ...
     $insurance = "SELECT  insuranceName, patient, insuranceCover,remainingCost FROM insurance";
     echo "<div style=' float:left; width:40%;  padding:20px;'><table> <caption>INSURANCE TABLE</caption><col width='130'><col width='130'><col width='130'><col width='130'><col width='130'><tr><th>Insurance Name</th><th>Patient ID </th><th>Insurance cover</th><th>Remaining Cost</th></tr>";
     $res = $mysqli->query($insurance);
     if($res->num_rows>0){
       while($row=$res->fetch_assoc()){
           echo "<tr><td>" . $row['insuranceName'] . "</td><td> " .  $row['patient'] . "</td><td> " . $row['insuranceCover'] . "</td><td> " . $row['remainingCost'] . "</td></td>";
       }
     }else{
         echo "<h2 style='text-align:center;color: #666;'>No insurance.</h2>";
       }
       echo "</table></div>";
...
body {color: purple;background: blue; background: -webkit-linear-gradient(to right, #F15F79, #B24592); background: linear-gradient(to right, #ccccff, #ff9999); }
</style>
</body>
</html>
